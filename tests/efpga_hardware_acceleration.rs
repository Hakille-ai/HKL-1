...
fn test_subnetwork_stability_analyzer() {
    ...
    let synapse_data = [
        ...
        (
            ...
            10,
            FixedPoint::from_f32(0.050),
            300,
        ), // Unstable var (0.05 > 0.005)
        (
            NeuronId::new(3),
            NeuronId::new(4),
    ...
    assert_eq!(
        subnetwork.count, 2,
        "Only 2 stable immutable synapses must be frozen!"
    );
...
fn test_embedded_efpga_bitstream_encoding() {
    ...
    assert_eq!(
        bitstream.data[0], 0xEB,
        "Bitstream must start with sync byte 0xEB"
    );
